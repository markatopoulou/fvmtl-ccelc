#ifndef CAFFE_COST_SIGMOID_CROSS_ENTROPY_LOSS_LAYER_HPP_
#define CAFFE_COST_SIGMOID_CROSS_ENTROPY_LOSS_LAYER_HPP_
...
class CostSigmoidCrossEntropyLossLayer : public LossLayer<Dtype> {
 public:
  explicit CostSigmoidCrossEntropyLossLayer(const LayerParameter& param)
  ...
  virtual inline const char* type() const { return "CostSigmoidCrossEntropyLoss"; }
 ...
 protected:
  ...
  /**
   * @brief Computes the cost sigmoid cross-entropy loss error gradient w.r.t. the
   *        predictions.
   *
  ...
  float cor_threshold;
  const Dtype* correlations;
  bool firstTime;
};

}  // namespace caffe

#endif  // CAFFE_COST_SIGMOID_CROSS_ENTROPY_LOSS_LAYER_HPP_
