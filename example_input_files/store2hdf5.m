function [curr_dat_sz, curr_lab_sz] = store2hdf5(filename, data, labels, create, startloc, chunksz, data_format, label_format)  
...
%     if ~exist('chunksz', 'var')
    ...
    h5create(filename, data_format, [dat_dims(1:end-1) Inf], 'Datatype', 'single', 'ChunkSize', [dat_dims(1:end-1) chunksz]); % width, height, channels, number 
    h5create(filename, label_format, [lab_dims(1:end-1) Inf], 'Datatype', 'single', 'ChunkSize', [lab_dims(1:end-1) chunksz]); % width, height, channels, number 
    if isempty(startloc) 
...
%     if ~exist('startloc','var')
  ...
  if ~isempty(data)
    h5write(filename, data_format, single(data), startloc.dat, size(data));
    h5write(filename, label_format, single(labels), startloc.lab, size(labels));  
  end
